class Graph(object):
    ...
    def add_edge(self, src, dst, cost,dir):
        """
    ...
    def initialize_graph(self, V, edge_list):
        ...
        for node in edge_list:
          self.add_edge(node[0],node[1],node[2],node[3])
...
def read_input(E):
    ...
    for _ in range(E):
       src, dst, cost, dir = input('').rstrip('\r\n').split()
       edge_list.append((int(src),int(dst),int(cost),int(dir)))
...
if __name__ == '__main__':
    ...
    V, E, En, X = input('').rstrip('\r\n').split()
    V, E, En, X = int(V), int(E), int(En), int(X)
